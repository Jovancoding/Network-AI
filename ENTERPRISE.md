...
| ClawHub Security Scanner | ✅ 3 Notes acknowledged and mitigated — ASI03 (advisory token identity, by design) and ASI06 ×2 (persistent context and audit-log PII risk, by design). Controls: advisory-token warning in SKILL.md, `_validate_context()` injection detection, PII warning on justification fields. Notes recur on every scan by design; documented controls are the mitigation. |
