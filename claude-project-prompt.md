...
> v5.15.1 — fixes GHSA-743h-jr5x-mpcr (ClaudeHookBridge full-target deny/allow matching) and GHSA-9v4f-j8cv-fhxw (SandboxPolicy canonicalized command matching); adds ContextComposer + `context_pack` / `blackboard_search` MCP tools (token-budgeted, relevance-ranked context — signal over noise); Gemini / OpenAI Responses / Claude Agent SDK adapters, Claude Code hooks gating (AuthGuardian), MCP elicitation approvals, and A2A server mode; includes model-interaction lifecycle governance (refusal → fallback, effort policy, OWASP Agentic Top 10 coverage).
