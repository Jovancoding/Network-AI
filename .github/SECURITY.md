...
| Version | Supported |
|---------|-----------|
| 5.15.x  | ✅ Yes — full support (current) |
...
- **ClaimVerifier — Tier 1 Agent Honesty** (v5.11.0) — `ClaimVerifier` (`lib/claim-verifier.ts`) reconciles agent-declared `ActionManifest[]` against runtime-witnessed `RuntimeAuditEntry` records using outcome-bound HMAC-signed `ExecutionReceipt`s. `AgentRuntime` issues receipts committing to `{ agentId, action, target, exitCode, outputHash }` — never the agent. Tampering with any field invalidates the signature. `UNSUPPORTED_CLAIM` / `UNDISCLOSED_ACTION` violations surface through `ComplianceMonitor`; repeated fabrication decays `AuthGuardian` trust and forces `ApprovalGate` supervision.
...
- **LockedBlackboard Mutex Correctness** (v5.8.6) -- five correctness fixes: stale-lock compare-and-delete (`forceReleaseStale()`); ownership-verified `release()` unlink; atomic snapshot via tmp+rename; WAL/pending zombie-entry reconciliation; priority-aware pending eviction protecting approval-gate proposals
...
- **Streamable HTTP MCP Transport Fail-Closed** (v5.11.0) — `McpStreamableServer` enforces a non-empty bearer secret before `listen()` will bind; empty secret causes immediate rejection, consistent with the SSE transport policy (GHSA-j3vx-cx2r-pvg8).
- **SkillSpector (v5.11.0) — `check_permission.py` two findings resolved**: `FILE_EXPORT` added to `HIGH_RISK_RESOURCES` (was missing despite comment/policy requiring `--confirm-high-risk`); `ensure_data_dir()` now delegates to `_resolve_data_dir()` for env-scoped correctness.
- **CodeQL #174 (CWE-377) — Insecure temporary file path in `AuthGuardian`** (v5.11.0): All `join(tmpdir(), ...)` sources in `test-claim-verifier.ts` replaced with `join('.', 'data', ...)` paths, eliminating the taint source. `AuthGuardian` constructor retains `path.resolve()` for defense-in-depth.
...
- **GHSA-743h-jr5x-mpcr** (CWE-436, CWE-863, High) — `ClaudeHookBridge`'s `extractTarget()` truncated the security-decision string to 500 characters while Claude Code executes the full, untruncated tool input, letting dangerous content past byte 500 evade the `denyPatterns` hard-block. **Fixed in v5.15.1** — deny/allow matching now runs against the full target; oversized targets (beyond a configurable `maxTargetLength`, default 65,536 chars) are denied outright rather than matched.
- **GHSA-9v4f-j8cv-fhxw** (CWE-436, CWE-863, High) — `SandboxPolicy`'s blocklist/allowlist/approval/risk matchers evaluated the raw, quote-and-whitespace-preserving command string while the executor tokenizes it (stripping quotes, collapsing whitespace) before `spawn(..., { shell: false })`; quoting a destructive or gated command (e.g. `rm -rf '/'`, `git "push" origin main`) evaded the blocklist and the human-approval gate while executing identically to its unquoted form. **Fixed in v5.15.1** — all matchers now operate on a canonicalized (`parseCommandLine` → `argv.join(' ')`) form matching exactly what the executor runs; unparseable input fails closed (approval required / high risk assessed).
