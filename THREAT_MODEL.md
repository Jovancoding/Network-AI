...
Version: 5.11.0 — updated with each release that changes a trust boundary or auth mechanism.
...
- `ShellExecutor` runs commands with `spawn(shell: false)` using a parsed argv; `SandboxPolicy` rejects unquoted shell metacharacters before the allowlist match, so a scoped allow such as `git *` cannot be escaped into arbitrary execution (v5.11.0, GHSA-qw6v-5fcf-5666).
...
### Tier 1 — What is built and enforced (v5.11.0+)
