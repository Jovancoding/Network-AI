...
- **ClaudeHookBridge Full-Target Deny/Allow Matching** (v5.15.1, GHSA-743h-jr5x-mpcr, CWE-436, CWE-863, High) — `extractTarget()` truncated the string used for the hook's security decision to 500 characters, while Claude Code executes the complete, untruncated tool input — dangerous content positioned past byte 500 was invisible to `denyPatterns` yet still ran, and `denyPatterns` is the sole hard-block in the default `observe` mode. **Fixed**: deny/allow pattern matching now runs against the full, untruncated target; targets exceeding a new `maxTargetLength` option (default 65,536 chars) are denied outright instead of matched, closing both the truncation bypass and unbounded-regex-cost DoS risk. Truncation is applied only afterward, for audit-log/display purposes.
- **SandboxPolicy Canonicalized Command Matching** (v5.15.1, GHSA-9v4f-j8cv-fhxw, CWE-436, CWE-863, High) — `isCommandAllowed`'s blocklist/allowlist glob, `requiresApproval`, and `assessRisk` matched the raw, quote-and-whitespace-preserving command string, while the executor tokenizes the command (stripping quotes, collapsing whitespace) before `spawn(..., { shell: false })`. Quoting a destructive command (e.g. `rm -rf '/'`) or a gated command (e.g. `git "push" origin main`) let it evade the blocklist safety net and the human-approval gate while executing identically to its unquoted form. **Fixed**: all three matchers now operate on a canonicalized form (`parseCommandLine` → `argv.join(' ')`) — the exact representation the executor runs — instead of the raw string; `requiresApproval`/`assessRisk` fail closed (require approval / assess high risk) on input that cannot be safely canonicalized.
